...
class Vehicle {
  ...
  applyBehaviors(vehicles, path) {
    ...
    // Cohere with other boids force
    let c = this.cohere(vehicles)
    // Align direction with other boids
    let a = this.align(vehicles)
    // Arbitrary weighting
    f.mult(3);
    c.mult(0.1);
    s.mult(1);
  ...
  // Alignment: move towards avg heading of local agents
  align(boids) {
    let localDistance = this.r * 3;
    let steer = createVector(0, 0);
    let count = 0;
    for (let i = 0; i < boids.length; i++) {
      let other = boids[i];
      let d = p5.Vector.dist(this.position, other.position);
      if (d > 0 && d < localDistance) {
  ...
  // Cohesion: move towards avg position of local agents
  cohere(boids) {
    let localDistance = this.r * 8;
    let steer = createVector(0, 0);
    let count = 0;
    for (let i = 0; i < boids.length; i++) {
      let other = boids[i];
      let d = p5.Vector.dist(this.position, other.position);
      if (d > 0 && d < localDistance) {
        // Calculate vector pointing towards neighbor
